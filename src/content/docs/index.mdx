...
Consider the following markup for a `<section>` element, which uses Toolbelt's [`x-section` directive](/components/section):
...
<section x-init x-section aria-labelledby="tb-section-title-1">
